...
async function ensureCollectionExists(collectionName) {
  ...
  try {
    ...
    if (!isCollectionNotExistsError(error)) {
      console.warn(`[admin] 检查集合 ${collectionName} 失败`, error);
      ensuredCollections.add(collectionName);
      return;
    }
  }
  ...
  try {
    ...
    console.error(`[admin] 创建集合 ${collectionName} 失败`, error);
    throw new Error(`创建集合 ${collectionName} 失败，请稍后重试或在云开发控制台手动创建。`);
  }
}

async function ensureProxyCollections() {
  await ensureCollectionExists(PROXY_SESSION_COLLECTION);
...
async function updateGlobalBackground(openid, config = {}) {
  ...
  if (!allowedIds.size) {
    if (nextGlobalBackground.enabled) {
      throw new Error('请先添加自定义背景');
    }
    nextGlobalBackground.backgroundId = '';
    nextGlobalBackground.animated = false;
  } else {
    if (!nextGlobalBackground.backgroundId || !allowedIds.has(nextGlobalBackground.backgroundId)) {
      nextGlobalBackground.backgroundId = catalog[0].id;
    }
    if (nextGlobalBackground.enabled && !nextGlobalBackground.backgroundId) {
      throw new Error('请选择要应用的背景');
    }
    if (
      nextGlobalBackground.animated &&
...
async function updateGlobalBackgroundCatalog(openid, updates = {}) {
  await ensureAdmin(openid);

  const requestedCatalog = Array.isArray(updates && updates.catalog)
    ? updates.catalog
    : updates;
  const desiredCatalog = normalizeBackgroundCatalog(requestedCatalog);
  ...
  const hasCatalogEntries = desiredCatalog.length > 0;

  if (!hasCatalogEntries) {
    nextGlobalBackground = { enabled: false, backgroundId: '', animated: false };
  } else if (!desiredCatalog.some((entry) => entry && entry.id === nextGlobalBackground.backgroundId)) {
    nextGlobalBackground.backgroundId = desiredCatalog[0].id;
    nextGlobalBackground.animated = false;
  }

  if (catalogUnchanged) {
    const backgroundUnchanged =
      nextGlobalBackground.enabled === currentToggles.globalBackground.enabled &&
      nextGlobalBackground.backgroundId === currentToggles.globalBackground.backgroundId &&
      nextGlobalBackground.animated === currentToggles.globalBackground.animated;
    if (backgroundUnchanged && existingDocument) {
      return {
        success: true,
        features: currentToggles,
        updatedAt: existingDocument.updatedAt || null
    ...
    .catch(async (error) => {
      if (isCollectionNotExistsError(error)) {
        ...
        await db
          ...
          .set({ data: sessionDoc })
          .catch((innerError) => {
            if (innerError && /already exist|duplicate/i.test(innerError.errMsg || '')) {
              return db
                .collection(PROXY_SESSION_COLLECTION)
                .doc(openid)
                .update({
  ...
  if (!existing) {
    await db
      ...
      .catch((error) => {
        ...
        console.error('[admin] 删除代理会话失败', error);
      });
...
function buildUpdatePayload(updates, existing = {}, extras = {}, levels = []) {
  ...
  if (Object.prototype.hasOwnProperty.call(updates, 'levelRewardResetLevel')) {
    ...
    if (Number.isFinite(rawReset)) {
      ...
      const deliveredClaims = normalizeClaimedLevelRewardsList(extras.deliveredLevelRewards);
      ...
      if (deliveredClaims.length) {
        const filteredDeliveredClaims = filterClaimedLevelRewardsByOrder(
          deliveredClaims,
          levelOrderMap,
          normalizedReset
        );
        if (!arraysEqual(filteredDeliveredClaims, deliveredClaims)) {
          extrasUpdates.deliveredLevelRewards = filteredDeliveredClaims;
        }
      } else if (!Array.isArray(extras.deliveredLevelRewards)) {
        extrasUpdates.deliveredLevelRewards = [];
      }
