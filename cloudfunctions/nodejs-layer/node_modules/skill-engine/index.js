...
const CONTROL_EFFECT_CONFIG = {
  stun: { summary: '被眩晕', skip: true, disableBasic: true, disableActive: true, disableDodge: true },
  silence: { summary: '被沉默', skip: false, disableBasic: false, disableActive: true, disableDodge: false },
  freeze: { summary: '被冰冻', skip: true, disableBasic: true, disableActive: true, disableDodge: true },
  sleep: { summary: '陷入沉睡', skip: true, disableBasic: true, disableActive: true, disableDodge: true }
};
...
function mergeControlConfig(target, effectKey) {
  const config = CONTROL_EFFECT_CONFIG[effectKey];
  if (!config) {
    return target;
  }
...
function resolveControlState(actor) {
  const state = { effects: [], skip: false, disableBasic: false, disableActive: false, disableDodge: false };
  ...
  statuses.forEach((status) => {
    ...
    mergeControlConfig(state, effect);
  });
...
function resolveControlSummary(effect) {
  ...
  return (CONTROL_EFFECT_CONFIG[key] && CONTROL_EFFECT_CONFIG[key].summary) || CONTROL_FALLBACK_SUMMARY;
}

function controlEffectCausesSkip(effect) {
  const key = normalizeControlEffect(effect);
  const config = CONTROL_EFFECT_CONFIG[key];
  return config ? !!config.skip : true;
}
...
function resolveResourceSetup({ combatant, stats, special }) {
  ...
  if (!Number.isFinite(resolvedStart)) {
    if (Number.isFinite(RESOURCE_CONFIG.startValue)) {
      resolvedStart = Math.round(Math.max(0, RESOURCE_CONFIG.startValue));
    } else {
...
function configureResourceDefaults(overrides = {}) {
  ...
  if (Number.isFinite(overrides.baseMax) && overrides.baseMax > 0) {
    RESOURCE_CONFIG.baseMax = Math.max(1, Math.round(overrides.baseMax));
  }
  if (Number.isFinite(overrides.startFraction)) {
    RESOURCE_CONFIG.startFraction = clamp(overrides.startFraction, 0, 1);
  }
  if (Number.isFinite(overrides.startValue)) {
    RESOURCE_CONFIG.startValue = Math.max(0, overrides.startValue);
  }
  if (Number.isFinite(overrides.turnGain)) {
    RESOURCE_CONFIG.turnGain = Math.max(0, overrides.turnGain);
  }
  if (Number.isFinite(overrides.basicAttackGain)) {
    RESOURCE_CONFIG.basicAttackGain = Math.max(0, overrides.basicAttackGain);
  }
  if (Number.isFinite(overrides.damageTakenGain)) {
    RESOURCE_CONFIG.damageTakenGain = Math.max(0, overrides.damageTakenGain);
  }
  if (Number.isFinite(overrides.critGain)) {
    RESOURCE_CONFIG.critGain = Math.max(0, overrides.critGain);
  }
  if (Number.isFinite(overrides.critTakenGain)) {
    RESOURCE_CONFIG.critTakenGain = Math.max(0, overrides.critTakenGain);
  }
  if (typeof overrides.type === 'string' && overrides.type.trim()) {
    RESOURCE_CONFIG.type = overrides.type.trim();
  }
  return { ...RESOURCE_CONFIG };
}
...
function executeSkill({ actor, opponent, skill, rng }) {
  ...
  if (skill.control) {
    const effect = applyControl(opponent, skill.control, skill.id, level, random);
    if (effect) {
