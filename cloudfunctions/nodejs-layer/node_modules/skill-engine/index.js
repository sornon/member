...
function resolveActorResourceGain(actor, key) {
  ...
  if (property && Number.isFinite(actor[property])) {
    return Math.max(0, Math.round(actor[property]));
  }
  return RESOURCE_CONFIG[key];
}
  ...
  statuses.forEach((status) => {
    ...
    switch (status.type) {
      ...
      case 'dot': {
        ...
        const damageGain = resolveActorResourceGain(actor, 'damageTakenGain');
...
function executeSkill({ actor, opponent, skill, rng }) {
  ...
  damageDescriptors.forEach((descriptor) => {
    ...
    for (let i = 0; i < hits; i += 1) {
      ...
      const opponentDamageGain = resolveActorResourceGain(opponent, 'damageTakenGain');
      ...
      if (defenderExtra.reflectRatio) {
        ...
        if (reflected > 0) {
          ...
          const reflectDamageGain = resolveActorResourceGain(actor, 'damageTakenGain');
  ...
  if (Array.isArray(skill.conditionalDamage)) {
    skill.conditionalDamage.forEach((entry) => {
      ...
      const conditionalDamageGain = resolveActorResourceGain(opponent, 'damageTakenGain');
      ...
      if (applied > 0) {
        ...
        if (defenderExtra.reflectRatio) {
          ...
          if (reflected > 0) {
            ...
            const followUpReflectGain = resolveActorResourceGain(actor, 'damageTakenGain');
        ...
        const followUpDamageGain = resolveActorResourceGain(opponent, 'damageTakenGain');
