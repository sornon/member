...
function clearControlEffects(actor, effect) {
  ...
  actor.controlState = resolveControlState(actor);
...
function applyIncomingDamageModifiers(target, damage, { element, summary } = {}) {
  ...
  const freezeConfig = CONTROL_EFFECT_CONFIG.freeze;
...
function handleDamageWakeEffects(target, damage, { summary, reason } = {}) {
  ...
  const sleepConfig = CONTROL_EFFECT_CONFIG.sleep;
...
function executeSkill({ actor, opponent, skill, rng }) {
  ...
  const shouldSuppressDodge = () => {
    ...
    const controlState = opponent.controlState || resolveControlState(opponent);
    if (opponent.controlState !== controlState) {
      opponent.controlState = controlState;
    }
    return !!(controlState && controlState.disableDodge);
  };
