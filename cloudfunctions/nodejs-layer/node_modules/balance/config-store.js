...
async function writeBalanceConfig(status, config = {}, metadata = {}) {
  const now = new Date();
  const existing = await findBalanceDocumentByStatus(status);
  ...
  await db.collection(BALANCE_CONFIG_COLLECTION).doc(String(nextId)).set({ data: payload });
