...
| 属性名 | 中文含义 | 默认值 | 备注 |
| --- | --- | --- | --- |
| `minAttackRatio` | 伤害计算中攻击力占比的最低阈值 | `0.25` | 防止低攻击系数技能伤害过低 |
| `randomMin` | 随机浮动区间的起始值 | `0.9` | 伤害随机波动下限 |
| `randomRange` | 随机浮动幅度 | `0.2` | 伤害随机波动范围 |
| `minDamage` | 伤害保底值 | `1` | 确保极端情况下仍有伤害 |

### finalDamage（终伤截断）

| 属性名 | 中文含义 | 默认值 | 备注 |
| --- | --- | --- | --- |
| `minMultiplier` | 终伤总乘区的最低倍率 | `0.1` | 终伤不会低于 10% 以防公式失真 |
| `bonusClamp.min` | 终伤加成的最小截断值 | `-0.9` | 终伤加成最多被压到 -90%，避免叠加减益导致过低 |
| `bonusClamp.max` | 终伤加成的最大截断值 | `2` | 终伤加成封顶为 +200%，限制极端增益 |
| `reductionClamp.min` | 终伤减免的最小截断值 | `0` | 不允许出现负减免 |
| `reductionClamp.max` | 终伤减免的最大截断值 | `0.9` | 终伤减免最高 90%，避免无伤情形 |

### healing（治疗/吸血限制）

| 属性名 | 中文含义 | 默认值 | 备注 |
| --- | --- | --- | --- |
| `lifeStealMax` | 吸血比例上限 | `0.6` | 吸血最高 60%，防止无限续航 |
| `healingBonusClamp.min` | 治疗加成的最小截断值 | `-1` | 治疗可被完全反转至 -100% |
| `healingBonusClamp.max` | 治疗加成的最大截断值 | `1.5` | 治疗增益封顶 150% |
| `healingReductionClamp.min` | 治疗减免的最小截断值 | `-1` | 治疗减免可被反向影响（最低 -100%） |
| `healingReductionClamp.max` | 治疗减免的最大截断值 | `1.5` | 治疗减免封顶 150% |
| `healingReceivedClamp.min` | 受治疗加成的最小截断值 | `-0.5` | 受治疗最低保留 50% 效果 |
| `healingReceivedClamp.max` | 受治疗加成的最大截断值 | `1.5` | 受治疗最多放大到 150% |

### procCaps（触发类上限）

| 属性名 | 中文含义 | 默认值 | 备注 |
| --- | --- | --- | --- |
| `comboRateMax` | 连击概率上限 | `1` | 触发概率最高 100% |
| `blockMax` | 格挡概率上限 | `1` | 格挡概率最高 100% |
| `counterRateMax` | 反击概率上限 | `1` | 反击概率最高 100% |

### specialCaps（特殊效果上限）

| 属性名 | 中文含义 | 默认值 | 备注 |
| --- | --- | --- | --- |
| `dodgeChanceMax` | 闪避概率上限 | `0.8` | 额外闪避最高 80% |
| `damageReflectionMax` | 反弹伤害比例上限 | `0.8` | 反伤比例最高 80% |
