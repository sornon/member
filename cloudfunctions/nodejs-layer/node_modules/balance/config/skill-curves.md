...
| 属性名 | 中文含义 | 默认值 | 备注 |
| --- | --- | --- | --- |
| `type` | 资源类型标识 | `"qi"` | 供技能消耗/回复引用的资源代号，决定 UI 与公式读取哪个槽 |
| `baseMax` | 最大资源值 | `100` | 资源槽上限，所有回复都会截断到此上限 |
| `startFraction` | 开局按最大值比例获得的资源 | `0`（`v2` 为 `0.1`） | v1 开局无额外资源；v2 给予 10% 上限起手，避免首回合干等 |
| `startValue` | 开局固定资源值 | `0` | 开局的固定资源补足，用于与比例叠加形成起手值 |
| `turnGain` | 每回合自然回复 | `20`（`v2` 为 `24`） | 回合结束自动回复量，v2 提升保证循环速度 |
| `basicAttackGain` | 普攻后获得的资源 | `10`（`v2` 为 `12`） | 普攻命中后额外回复，v2 提升鼓励普攻穿插 |
| `damageTakenGain` | 受伤获得资源系数 | `1.5`（`v2` 为 `1.8`） | 按伤害量乘系数获得资源，受击越痛回复越多；v2 提升压缩资源贫乏感 |
| `critGain` | 造成暴击时额外获得的资源 | `1` | 暴击输出时额外奖励 1 点，鼓励暴击玩法 |
| `critTakenGain` | 被暴击时额外获得的资源 | `1` | 被暴击的补偿资源，缓冲暴击带来的节奏损失 |
...
| `stun` | `skip` | 是否跳过整回合 | `true` | 回合直接被跳过，提供强控制感 |
| `stun` | `disableBasic` | 禁用普攻 | `true` | 眩晕下不可普攻，避免触发额外收益 |
| `stun` | `disableActive` | 禁用主动技能 | `true` | 眩晕下不可释放技能，彻底封锁行动 |
| `stun` | `disableDodge` | 禁用闪避 | `true` | 眩晕下不可闪避，保证控制后的命中 |
| `silence` | `summary` | 状态描述 | `"沉默"` | 显示用文案 |
| `silence` | `skip` | 是否跳过整回合 | `false` | 仍可行动，只禁技能，突出“封技能不封移动” |
| `silence` | `disableBasic` | 禁用普攻 | `false` | 普攻可用，保证最低输出循环 |
| `silence` | `disableActive` | 禁用主动技能 | `true` | 主动技能不可用，核心作用在于打断技能链 |
| `silence` | `disableDodge` | 禁用闪避 | `false` | 闪避正常，可靠闪避暂时规避伤害 |
| `freeze` | `summary` | 状态描述 | `"冰冻"` | 显示用文案 |
| `freeze` | `skip` | 是否跳过整回合 | `true` | 冰冻回合会跳过，等同强控 |
| `freeze` | `disableBasic` | 禁用普攻 | `true` | 冰冻下不可普攻，冻结动作 |
| `freeze` | `disableActive` | 禁用主动技能 | `true` | 冰冻下不可释放技能，冻结技能释放 |
| `freeze` | `disableDodge` | 禁用闪避 | `true` | 冰冻下不可闪避，便于后续打击 |
| `freeze` | `breakOnFire` | 受到火属性伤害是否解除 | `true` | 火属性伤害可融解冰冻，提供克制解法 |
| `freeze` | `fireDamageMultiplier` | 火属性克制时的额外伤害系数 | `0.1` | 冰冻被火克制时额外 +10% 伤害，体现属性相克 |
| `sleep` | `summary` | 状态描述 | `"陷入沉睡"` | 显示用文案 |
| `sleep` | `skip` | 是否跳过整回合 | `true` | 睡眠回合会跳过，软控但可被提前打醒 |
| `sleep` | `disableBasic` | 禁用普攻 | `true` | 睡眠下不可普攻 |
| `sleep` | `disableActive` | 禁用主动技能 | `true` | 睡眠下不可释放技能 |
| `sleep` | `disableDodge` | 禁用闪避 | `true` | 睡眠下不可闪避 |
| `sleep` | `wakeOnDamage` | 受击是否立即醒来 | `true` | 受到任意伤害会解除，便于平衡持续时间 |
| `sleep` | `turnResourceGain` | 睡眠回合仍然被动回复的资源量 | `10`（`v2` 为 `14`） | 控制下仍保留被动回能，v2 提升让睡眠后续能更快开技能 |
