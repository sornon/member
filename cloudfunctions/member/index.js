...
const { applyLevelGrantToProfile } = require('../pve/index.js');
const {
...
async function applyLevelGrantRewards(openid, level, levels = []) {
  ...
  if (hasInventoryRewards) {
    const { profile: updatedProfile, changed } = applyLevelGrantToProfile(
      member.pveProfile || {},
      level.grants,
      { now }
    );
    if (changed) {
      memberUpdates.pveProfile = _.set(updatedProfile);
    }
  }
