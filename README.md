...
- **宗门社交与团队讨伐**：新增宗门（公会）系统，支持创建/加入宗门、排行榜缓存与团队讨伐玩法，详情见 [宗门系统](docs/guild.md)，风控参数与错误码分别集中在 `cloudfunctions/guild/constants.js` 与 `cloudfunctions/guild/error-codes.js` 中管理。
