...
| `applyLevelGrant` | 根据传入的等级奖励配置，直接在会员档案中发放装备、技能与储物道具，供会员等级体系调用。发放逻辑已经拆分到公共模块 `pve-level-grant`，`member` 云函数直接引用该模块即可，无需跨云函数调用。 |
...
2. 同步更新 `member` 云函数，并上传公共模块 `pve-level-grant`（目录 `cloudfunctions/nodejs-layer/pve-level-grant`），确保等级奖励发放逻辑保持最新。当前版本不再跨云函数调用 `pve`，仅依赖公共模块即可。
