...
- **2025-10-27：秘境首领立绘被放大**。战斗舞台模板会把 `figureScaleClass` 拼接在双方立绘容器上，`figure-scale--ss` 在全局样式中将立绘放大到 1.15 倍。【F:miniprogram/shared/templates/battle-stage.wxml†L190-L199】【F:miniprogram/app.wxss†L107-L120】结构化战斗视图由 `buildStructuredBattleViewModel` 生成，它会调用 `resolveFigureRarityFromCandidates` 遍历上下文中与稀有度或段位相关的字段（如 `opponentTier`、`enemyPreview`、`battle.enemy` 等），并通过 `normalizeFigureRarity` 判断字符串是否包含 `ss`/`sss` 来推断稀有度；秘境首领的段位标记为 `boss`，字符串含有 “ss”，因此被归类为 `SS` 稀有度并映射到 `figure-scale--ss`。【F:miniprogram/shared/battle.js†L256-L433】【F:miniprogram/shared/figure-scale.js†L1-L36】【F:cloudfunctions/pve/index.js†L7041-L7105】修复后，`buildStructuredBattleViewModel` 会汇总秘境对手的楼层信息，仅在楼层为 10 的倍数时保留 `figure-scale--ss`，避免普通层 NPC 被误判为首领导致放大样式叠加。【F:miniprogram/shared/battle.js†L339-L433】【F:miniprogram/shared/battle.js†L2399-L2436】
- **2025-10-28：PVE 接口调用超时**。线上云函数频繁返回 `FUNCTIONS_TIME_LIMIT_EXCEEDED`，日志显示 `profile` 动作耗时接近 3 秒才被云开发强制中断。排查确认瓶颈来自秘境列表渲染：若在 `decorateSecretRealm` 的楼层循环中逐个计算玩家战力并让 `decorateEnemy` 重新格式化每个怪物的掉落/奖励，就会在一次请求中触发 90 余次数值汇总与文本拼接，CPU 耗时随玩家战力提高而线性上升。【F:cloudfunctions/pve/index.js†L6891-L6901】【F:cloudfunctions/pve/index.js†L7041-L7105】
- **修复**：在模块初始化阶段预先缓存怪物的战力、奖励文本、掉落与管理员预览，并在装饰楼层前一次性计算玩家战力；渲染时直接复用缓存并克隆返回对象，避免重复的 `calculateCombatPower` 和格式化流程。无需额外部署操作，只需重新上传 `pve` 云函数代码即可生效。【F:cloudfunctions/pve/index.js†L2743-L2790】【F:cloudfunctions/pve/index.js†L6891-L6901】【F:cloudfunctions/pve/index.js†L7041-L7105】
