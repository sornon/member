...
3. **设置环境变量**：
   - 如果已经将微信支付下载的 `apiclient_key.pem`、`apiclient_cert.pem` 复制到 `cloudfunctions/wallet/` 与 `cloudfunctions/wallet-pay-notify/` 目录，云函数会在运行时自动读取这些文件，无需再为私钥与平台证书配置环境变量，只要在部署时保证文件被一并上传。
   - 若仍希望通过环境变量托管敏感信息，可进入「云函数」> 选择 `wallet` 与 `wallet-pay-notify` 两个函数 > 「环境变量」，并新增或更新以下键值对（所有键名区分大小写）：

     | 变量名 | 含义 | 建议值/格式 |
     | --- | --- | --- |
     | `WECHAT_PAY_API_V3_KEY` | APIv3 密钥 | 32 位纯文本 |
     | `WECHAT_PAY_SERIAL_NO` | 商户证书序列号 | 16 进制字符串（不含冒号） |
    | `WECHAT_PAY_PRIVATE_KEY` | 商户私钥内容 | 将 `apiclient_key.pem` 全量粘贴，可保留换行；若复制的是 `\n` 转义形式程序会自动还原，勿额外包裹引号。若目录中已放置同名文件可留空 |
    | `WECHAT_PAY_PLATFORM_CERT` | 微信支付平台证书内容 | 同样完整粘贴 PEM 内容，支持 Base64 或带 `\n` 转义的格式。若目录中已放置同名文件可留空 |
...
> **安全提示**：证书私钥与密钥均属于敏感信息。若采用文件方式部署，请确保 `apiclient_key.pem`、`apiclient_cert.pem` 未被提交到公开仓库（可在 `.gitignore` 中忽略），并限制下载权限；若通过环境变量托管，同样要避免在日志中输出完整内容。证书轮换后记得同步更新 `WECHAT_PAY_SERIAL_NO` 以及对应的文件或环境变量。
