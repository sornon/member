...
会员修为累计至 180000（炼气圆满）后，无需点击“突破境界”按钮，权益页便自动出现“任意饮品券（120 元内）”权益，导致突破奖励提前到账。

**原因分析**

充值回调与后台等级同步会调用 `syncMemberLevel`，遍历等级文档中的 `level.rewards` 自动派发权益。突破奖励配置使用与常规奖励相同的字段，且同步函数未过滤突破专属权益，因而在炼气圆满时就把饮品券权益写入会员权益集合。

**修复方案**

公共模块 `common-config` 新增 `resolveRegularLevelRights`，识别并剔除突破权益 ID。钱包支付回调、钱包余额同步及后台等级同步均改用该方法，仅派发常规等级权益；突破奖励仍由云函数 `member` 的 `breakthrough` 接口在达成条件并点击按钮后发放。
