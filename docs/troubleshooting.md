...
领取奖励会刷新仓库内全部装备的 `updatedAt/obtainedAt` 时间戳。前端的角标状态机会把“最新时间戳晚于上次已读时间”的物品视为未读，从而把整仓装备误判为新物品。

**修复方案**

自本次提交起，前端在同步仓库状态时，会在确认物品未带有 `isNew`/`hasNewBadge` 标记的情况下，把刷新后的时间戳写回已读记录，避免旧装备被误判为新装备。仅有真正带 `isNew` 标记的奖励会继续显示“新”角标，无需额外操作即可恢复正常。【F:miniprogram/utils/storage-notifications.js†L350-L368】
