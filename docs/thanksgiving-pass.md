...
- 修复后台已更新门票总数但前台仍显示旧值的问题：此前 `bhkBargainStock` 库存文档只在首次创建时写入 `totalStock`，后续活动管理后台修改 `bargainSettings.stock` 不会回刷该文档，导致前台持续读取旧库存（如仍显示 15）。现已在 `ensureBargainStockDoc` 中增加配置对齐：当检测到配置总库存变更时，会按 `sold` 重算并更新 `totalStock/stockRemaining`，保证前台展示与后台配置同步。
