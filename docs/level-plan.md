...
2. **重新部署相关云函数**：提交修改后需要同步上传 `bootstrap`、`admin`、`menuOrder`、`wallet` 等引用该配置的云函数，确保运行时代码读取到最新参数。涉及等级奖励的改动时，请同时上传最新的 `member` 与 `pve` 云函数，当前版本会通过远程调用 `pve.applyLevelGrant` 发放装备/技能，避免旧包继续尝试 `require('../pve/index.js')` 导致报错。
