...
| 小等级 | 奖励内容 | 功能说明 |
| --- | --- | --- |
| 炼气一层 | 称号「炼气新人」 | 默认授予并在用户未设置称号时自动展示，凸显修行入门身份。 |
| 炼气二层 | 背景「灵力测试」 | 解锁后可在首页 → 头像 → 外观设置中切换场景背景。 |
| 炼气三层 | 武器「青竹练气杖」 | 加入装备背包，可在角色 → 装备页进行穿戴。 |
| 炼气四层 | 技能「烈炽火弹」 | 直接加入技能仓库，可在角色 → 技能页装备上阵。 |
| 炼气五层 | 衣装「初阳布袍」 | 新增衣装类装备，提供基础生命与防御加成。 |
| 炼气六层 | 鞋履「翎痕轻履」 | 获得轻履装备，提升闪避属性。 |
| 炼气七层 | 腰带「知风木环」 | 获得腰带装备，增强治疗与受疗能力。 |
| 炼气八层 | 护腕「回鸣臂缚」 | 获得护腕装备，附带反击类增益。 |
| 炼气九层 | 法器「清魂定珠」 | 获得法器装备，提升控制命中率。 |
| 炼气圆满 | 道具「技能5连抽」 | 奖励消耗品存入纳戒（角色 → 纳戒页），使用后立即连续抽取 5 张技能卡。 |

> 达到炼气十层（圆满）后，修为继续累积不会自动进入下一境界，需在角色界面执行“突破”操作后才能解锁筑基期相关内容。

> 「技能5连抽」在纳戒列表的道具分类中可直接使用，系统会同步新增抽取到的技能并弹出提示。
...
| 主境界 | 境界突破奖励 | 实体/服务权益 |
| --- | --- | --- |
| 炼气圆满 | 背景「筑基背景」 | 任意 ¥120 内饮品权益券 1 张 |
...
2. **重新部署相关云函数**：提交修改后需要同步上传 `bootstrap`、`admin`、`menuOrder`、`wallet` 等引用该配置的云函数，确保运行时代码读取到最新参数。涉及等级奖励的改动时，请同时上传最新的 `member` 云函数，并同步更新公共模块 `pve-level-grant`（位于 `cloudfunctions/nodejs-layer/pve-level-grant`），云函数在运行时直接依赖该模块以发放装备/技能，无需再跨云函数调用 `pve`。
