...
| 成员战力提升后宗门战力仍未更新 | 仅在建帮/入帮/退帮时变更 `guilds.power`，后续成员战力调整未做增量同步 | 升级 `guild-service`：在获取宗门概览时通过 `syncMemberPower` 对比成员当前战力与存档差值，自动更新 `guildMembers` 与 `guilds.power` 并刷新排行榜缓存；历史数据可手动按成员战力汇总回填后再调用接口触发同步。|
