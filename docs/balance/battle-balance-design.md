...
- **运行时 Loader**：`cloudfunctions/nodejs-layer/balance/config-loader.js` 提供 `getLevelCurveConfig` 等方法，以及 `setBalanceVersion/getBalanceVersion`、`__resetBalanceCache`（测试用）。缺失字段自动回退到旧版常量并打印警告。
- **战斗模拟入口**：`cloudfunctions/nodejs-layer/balance/simulator.js` 提供 `simulatePveBattle`、`simulatePvpBattle`，使用现有战斗引擎（combat-system / skill-engine），可带 seed 复现。
- **自动化测试**：
