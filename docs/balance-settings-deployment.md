...
- **暂存与测试**：点击「暂存配置」写入 `balanceConfigs` 集合的 `status=staging` 记录，不影响线上；「测试暂存配置」基于暂存数据跑多轮 PVP PK，生成胜率/回合数报告。
- **应用到全局**：对测试结果满意后点击「应用到全局」，会自动将暂存记录升级为 `status=use`，并把上一条 `status=use` 记录标记为 `status=backup`，实时刷新 PVE/PVP 云函数的运行时缓存。
