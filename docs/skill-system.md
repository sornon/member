...
#### 真气基础参数（对齐《寻道大千》）
- **资源上限与起手值**：战斗双方默认拥有 100 点真气上限，并以 50%（50 点）真气开局；后续所有结算均会在 0~上限区间内自动截断，避免溢出或负值。
- **回合起始回复**：每当角色轮到自身行动前，固定恢复 20 点真气，并乘以当前 `rageGain`（真气回复系数）加减成效；若系数被削减至 0 以下，则按 0 处理，不会产生负回复。
- **普攻与受击增益**：普攻命中后额外获得 20 点真气；角色受到任何直接伤害（含连击、条件伤害、反震与持续伤害）时会补充 10 点真气，同样受 `rageGain` 放大或缩小。
