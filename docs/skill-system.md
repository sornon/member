...
| 控制类型 | 战斗提示 | 行动影响 |
| --- | --- | --- |
| 眩晕（`stun`） | 被眩晕 | 跳过本回合行动，无法普攻、无法释放主动攻击技能，且无法闪避后续攻击。 |
| 冰冻（`freeze`） | 被冰冻 | 与眩晕相同：直接跳过行动，期间无法普攻、无法释放主动攻击技能，也无法闪避。 |
| 沉睡（`sleep`） | 陷入沉睡 | 跟随回合跳过规则，与眩晕一致，期间既不能行动也不能闪避，直至状态结束或被唤醒。 |
| 沉默（`silence`） | 被沉默 | 保留普攻与闪避能力，但禁止释放所有主动攻击技能。 |
