...
- **快速匹配与好友对战**：支持按积分区间自动匹配、从排行榜入口直接切磋榜单成员以及通过分享链接发起切磋。若暂时缺乏真人对手，会自动生成平衡的 AI 傀儡。除自动匹配外，其余模式均视为演练赛，不会结算赛季积分与胜负战绩；排行榜切磋次数不限，胜负也不会计入战绩。
...
- **安全与风控**：战斗计算固定随机种子并由服务端结算，好友切磋同样在云端模拟且标记为演练赛，防止刷分与恶意篡改。

## 匹配与积分规则

- **积分区间匹配**：快速匹配会以当前积分为中心，取最大值为“积分的 10%”与“150 分”两者中的较大者作为搜索半径，仅挑选积分落在 `[自身积分-半径，自身积分+半径]` 内的对手，并优先选择最近一段时间未参战的玩家；若没有真人候选，就生成属性与积分接近的傀儡守卫参战。【F:cloudfunctions/pvp/index.js†L296-L365】【F:cloudfunctions/pvp/index.js†L1332-L1385】
- **机器人对手的构成**：傀儡档案会按 0.9~1.1 的随机系数调整真实玩家的当前积分、胜负与连胜等统计数据，同时继承其战斗快照，保证战力接近且不影响排行榜真实排序。【F:cloudfunctions/pvp/index.js†L1368-L1385】
- **积分结算公式**：只有天梯匹配会变动积分，好友切磋与邀战一律视作演练赛；系统在结算时以 30（胜）、-22（负）、8（平）为基准分，再根据双方积分差（最多 ±1.5 系数）和当前连胜（最多 +10 分）计算结果，最终积分不会低于当前段位下限。对战傀儡时，胜场加分上限 20、失利扣分下限 -10，避免通过机器人刷分。【F:cloudfunctions/pvp/index.js†L1242-L1330】
- **基础段位与赛季起点**：所有新参赛者默认从 1200 分的白银段位起步，赛季段位区间为青铜至宗师六档，对应不同的奖励配置。【F:cloudfunctions/pvp/index.js†L50-L70】
- **战斗限制**：同一账号相邻两场对战至少间隔 10 秒，系统在进入匹配或邀战前会检测冷却；正式对战最多持续 15 回合，到达上限仍未分出胜负即判定为平局。【F:cloudfunctions/pvp/index.js†L46-L52】【F:cloudfunctions/pvp/index.js†L175-L199】【F:cloudfunctions/pvp/index.js†L296-L365】
- **好友切磋链接**：分享链接直接携带目标成员 ID，打开后立即发起切磋，无需生成邀战令，也没有有效期限制，每次都会读取双方的最新档案信息。【F:miniprogram/pages/pvp/index.js†L286-L319】【F:miniprogram/pages/pvp/index.js†L40-L82】
...
| `pvpInvites` | 旧邀战令数据，用于兼容历史分享记录；当前分享切磋已直接携带成员 ID。 |
...
| `sendInvite` | 生成邀战令并返回分享参数（兼容旧版本），新版本分享直接走 `matchFriend`。 |
| `acceptInvite` | 接受邀战令并模拟战斗，同时更新邀战状态与战报（兼容旧版本）。 |
...
- 防刷机制：好友切磋标记为演练赛且不结算积分，所有战斗结果都会生成 MD5 签名返回前端；机器人对战的积分增减有限制，避免刷分。
...
- `index`：竞技场主界面，显示当前段位、胜负统计、最新战报与历史赛季记录，可进行快速匹配、领取奖励并一键分享切磋链接。
- `leaderboard`：天梯排行榜，支持一键切磋或跳转回主界面发起邀战；比武榜条目若缺少头像则回退到默认头像，自身条目不显示切磋按钮，并禁止对自己发起切磋。
...
7. 从旧版本升级到支持排行榜称号解析的版本时，请额外刷新一次排行榜缓存。
   ...
   - 在动作（`action`）字段中填入 `getLeaderboard`，如需显式指定赛季或条数，可在 JSON 中增加 `seasonId`、`limit` 等可选参数；若仅为刷新缓存，可保持默认。
   - 将 `event` 的 JSON 负载设置为 `{"action":"getLeaderboard"}`，无需填写 `force` 等额外字段，随后点击「调用」；云函数会在检测到旧格式缓存时自动重建并写回 `pvpLeaderboard` 文档。无需带上登录信息即可刷新缓存，如需验证某个成员的排名，可额外在 JSON 中填入 `actorId`（或 `memberId`）。
