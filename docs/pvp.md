...
7. 从旧版本升级到支持排行榜称号解析的版本时，请额外刷新一次排行榜缓存：可在微信开发者工具的「云开发」面板中调用 `pvp` 云函数的 `getLeaderboard` 动作（或清空 `pvpLeaderboard` 集合），函数会自动重建缓存并补齐 `titleCatalog` 字段，避免前端拿到旧格式数据导致称号缺失。
