...
## 9. 多题配置说明
- 支持 `quizReward.questions` 数组。
- 每题答对奖励 +1 次，按题目粒度防重复（同题不可重复领奖，不同题可分别领奖）。
- 管理后台可通过“多题配置”文本区录入，格式：`题目|选项1|选项2|...|正确答案序号`（每行一题）。
- 前台以自定义弹层答题：含半透明遮罩、题目、选项、提交/取消；存在后续题目时显示“下一题”。
