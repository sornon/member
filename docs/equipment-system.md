...
| --- | --- | --- | --- |
| 青竹练气杖 | 炼气·初悟（1-3） | 基础攻击 +43.0 | 入门木杖，对应初入境界的基础物理需求。 |
| 赤铜灵锋 | 炼气·小成（4-6） | 最终增伤 +4.7% | 让玩家在中段体验增伤玩法，适配术法与物理双流派。 |
| 飞星散弩 | 炼气·圆满（7-10） | 连击率 +8.0% | 引导圆满阶段尝试连击触发，配合高悟性面板。 |

### 头部（主属性：物防 / 法防 / 格挡）

| 名称 | 推荐等级 | 主属性（凡品·阶段基准） | 定位说明 |
| --- | --- | --- | --- |
| 羊皮束额 | 炼气·初悟（1-3） | 物理防御 +28.9 | 贴合初段体质/根骨成长，缓和副本物理压力。 |
| 凝露纱冠 | 炼气·小成（4-6） | 法术防御 +39.5 | 针对灵力怪物，提供法防进阶选择。 |
| 烁纹面甲 | 炼气·圆满（7-10） | 格挡率 +9.6% | 搭配反击词条，提高圆满阶段的格挡上限。 |

### 衣服（主属性：生命% / 减伤 / 护盾强度）

| 名称 | 推荐等级 | 主属性（凡品·阶段基准） | 定位说明 |
| --- | --- | --- | --- |
| 初阳布袍 | 炼气·初悟（1-3） | 生命上限倍率 +7.5% | 结合基础生命公式，放大初段体质收益。 |
| 定岩护甲 | 炼气·小成（4-6） | 减伤 +11.1% | 提供中段副本所需的常驻减伤。 |
| 沁灵罩衣 | 炼气·圆满（7-10） | 护盾强度 +13.4% | 鼓励圆满阶段配合护盾技能稳固耐久。 |

### 鞋履（主属性：速度 / 闪避 / 命中）

| 名称 | 推荐等级 | 主属性（凡品·阶段基准） | 定位说明 |
| --- | --- | --- | --- |
| 踪风布鞋 | 炼气·初悟（1-3） | 速度 +23.0 | 结合敏捷成长，确保新手抢占先手。 |
| 翎痕轻履 | 炼气·小成（4-6） | 闪避 +23.5 | 让中段尝试闪避堆叠，配合身法加点。 |
| 乾衡行靴 | 炼气·圆满（7-10） | 命中 +24.5 | 圆满阶段补齐命中阈值，稳定控制链。 |

### 腰带（主属性：生命% / 受疗 / 治疗强化）

| 名称 | 推荐等级 | 主属性（凡品·阶段基准） | 定位说明 |
| --- | --- | --- | --- |
| 绳结束带 | 炼气·初悟（1-3） | 生命上限倍率 +6.8% | 与衣服搭配，帮助新手站稳前排。 |
| 知风木环 | 炼气·小成（4-6） | 受疗加成 +9.0% | 提升中段副本的治疗收益。 |
| 灵息法缠 | 炼气·圆满（7-10） | 治疗量加成 +9.8% | 让奶妈在圆满阶段补足治疗倍率。 |

### 护腕（主属性：基础攻击 / 反击率 / 格挡）

| 名称 | 推荐等级 | 主属性（凡品·阶段基准） | 定位说明 |
| --- | --- | --- | --- |
| 砭石护腕 | 炼气·初悟（1-3） | 基础攻击 +35.8 | 增补初段力量输出，让物理入门更顺手。 |
| 回鸣臂缚 | 炼气·小成（4-6） | 反击率 +10.6% | 与格挡流组合，提供中段反击尝试。 |
| 护山叶铠 | 炼气·圆满（7-10） | 格挡率 +9.6% | 圆满阶段兼顾格挡与反击，提升生存。 |

### 宝珠（主属性：法攻 / 控制命中 / 最终增伤）

| 名称 | 推荐等级 | 主属性（凡品·阶段基准） | 定位说明 |
| --- | --- | --- | --- |
| 琥珀聚灵 | 炼气·初悟（1-3） | 法术攻击 +41.2 | 配合法系成长，补足早期法攻基数。 |
| 清魂定珠 | 炼气·小成（4-6） | 控制命中 +33.6 | 让控制职业在中段堆出命中差。 |
| 炎脉注灵 | 炼气·圆满（7-10） | 最终增伤 +5.3% | 圆满阶段为术法输出提供增伤过渡。 |

### 项圈（主属性：控制命中 / 治疗强化 / 怒气获取）

| 名称 | 推荐等级 | 主属性（凡品·阶段基准） | 定位说明 |
| --- | --- | --- | --- |
| 灵纹索坠 | 炼气·初悟（1-3） | 控制命中 +26.2 | 满足初段控制命中阈值。 |
| 慈流颈环 | 炼气·小成（4-6） | 治疗量加成 +9.0% | 支撑治疗职业过渡。 |
| 斗志牙牌 | 炼气·圆满（7-10） | 怒气获取 +13.4% | 提升大招循环频率，配合圆满副本。 |

### 信物（主属性：全属性 / 减伤 / 护盾强度）

| 名称 | 推荐等级 | 主属性（凡品·阶段基准） | 定位说明 |
| --- | --- | --- | --- |
| 初悟令符 | 炼气·初悟（1-3） | 全属性 +9.0 | 平衡六维成长，强化基础面板。 |
| 碧甲战旗 | 炼气·小成（4-6） | 减伤 +10.6% | 中段副本的常驻减伤信物。 |
| 灵盾石佩 | 炼气·圆满（7-10） | 护盾强度 +13.4% | 与护盾技能联动，构筑团队防线。 |

### 傀儡（主属性：反击率 / 护盾强度 / 召唤物强度）

| 名称 | 推荐等级 | 主属性（凡品·阶段基准） | 定位说明 |
| --- | --- | --- | --- |
| 木魈守偶 | 炼气·初悟（1-3） | 反击率 +10.0% | 入门反击宠物，搭配格挡装备使用。 |
| 雾藤护灵 | 炼气·小成（4-6） | 护盾强度 +12.4% | 中段提供额外护盾支援。 |
| 羽翎侍从 | 炼气·圆满（7-10） | 召唤物强度 +13.5% | 圆满阶段让召唤流提前体验规模化。 |

### 法器（主属性：法攻 / 最终增伤 / 元素易伤）

| 名称 | 推荐等级 | 主属性（凡品·阶段基准） | 定位说明 |
| --- | --- | --- | --- |
| 霜毫法笔 | 炼气·初悟（1-3） | 法术攻击 +39.4 | 提供入门法器，巩固灵力输出。 |
| 银辉法鉴 | 炼气·小成（4-6） | 最终增伤 +5.0% | 术法增伤过渡装，配合中段技能。 |
| 烈脉骨铃 | 炼气·圆满（7-10） | 元素易伤 +11.5% | 让圆满阶段提前建立易伤循环。 |

### 秘宝（主属性：最终增伤 / 最终减伤 / 减伤）

| 名称 | 推荐等级 | 主属性（凡品·阶段基准） | 定位说明 |
| --- | --- | --- | --- |
| 破晓灵盘 | 炼气·初悟（1-3） | 最终增伤 +4.5% | 入门秘宝，帮助输出突破初段伤害线。 |
| 守心石印 | 炼气·小成（4-6） | 最终减伤 +3.8% | 中段抗压工具，降低群怪爆发。 |
| 流火坠玉 | 炼气·圆满（7-10） | 减伤 +11.3% | 圆满阶段的团队防御核心，补足高端副本需求。 |
