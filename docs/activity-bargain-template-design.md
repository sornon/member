...
### 修复：答题弹窗会自动关闭
**现象**：点击“提交答案”后（无论对错），弹窗很快自动关闭。

**原因**：提交后代码会立即再次请求 `bargainStatus`，请求返回后触发 `applySession`，把 `showQuizModal` 重置为 `false`。

**修复**：去掉提交答案后的自动刷新，改为由用户点击“我知道了/取消”关闭弹窗后再刷新状态。

#### 小白部署步骤
1. 上传小程序前端代码。
2. 开发者工具点击“清缓存并编译”。
3. 真机退出小程序后重进 `活动-test`。
4. 点击“去答题”→提交答案，确认弹窗不会自动关闭。
