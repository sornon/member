...
### 2.2 创建“答题加砍价次数”配置（手动）
1. 管理后台 → 活动管理 → 编辑目标 `concert-bargain` 活动。
2. 在砍价配置区开启 **启用答题奖励** 开关。
3. 答题奖励次数固定为 `1`（每答对一道题增加 1 次砍价机会，不可配置为其他值）。
4. 在 **题库(JSON)** 中维护题目，字段结构如下（可多题）：
   - `question`：题干
   - `options`：选项数组（建议写成 `A./B./C.` 格式）
   - `answer`：正确答案（例如 `C`）
   - `tips`：题目提示文案（无论答题是否正确都展示）
5. 若不需要答题玩法，直接关闭 **启用答题奖励**，此时无需配置题库（后端会自动按关闭态处理）。

### 2.3 本次答题能力发布步骤（必须按顺序）
1. **先部署云函数**：
   - 右键 `cloudfunctions/admin` → 上传并部署（云端安装依赖）
   - 右键 `cloudfunctions/activities` → 上传并部署（云端安装依赖）
2. **再上传小程序版本**：
   - 构建 npm（如项目启用）
   - 上传版本并填写更新说明（建议包含“concert-bargain 答题奖励配置”）
   - 提交审核/发布
3. **发布后验收**：
   - 管理端可见“启用答题奖励 / 题库(JSON)”两个配置项。
   - 开关关闭时可直接保存活动，不要求填写题目。
   - 开关开启且题库 JSON 非法时，保存会提示“答题题目格式错误，请检查JSON”。
