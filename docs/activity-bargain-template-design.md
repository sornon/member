...
1. 确认已合入最新 `cloudfunctions/activities/index.js` 修复（答题事务写库时会移除 `_id` 字段后再 `set`）。
2. 仅需重新部署 `cloudfunctions/activities`（此问题在云函数层，前端可不发版）。
3. 部署完成后重新进入“活动-test”答题砍价验证：
   - 不再出现 `_id` 更新报错；
   - 仍能正常返回“正确答案 + Tips”；
   - 首次答对题目可正常 +1 次砍价。
