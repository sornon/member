...
### 4. 数据指标
- 每次成交会累计写入 `tradeMetrics` 文档（`global`），记录成交笔数、成交额与手续费燃烧量，供后续统计分析。
- 管理员可通过后台交易行管理页面按时间倒序查看流水，并结合以上指标监控经济走势。
...
- `tradeSettings`：交易参数配置（手续费、挂单时长上下限等）。
...
2. **云函数部署**
   - 在微信开发者工具中分别选择 `trading` 与 `admin` 云函数，均需执行「上传并部署：云端安装依赖」。
   - 若使用 CI/CD，可在命令行执行 `npm install`（云端自动安装）后通过 `tcb` CLI 分别部署两项云函数。
...
5. **小程序前端**
   ...
   - 管理后台需新增上传发布 `subpackages/admin/trading/index` 页面，确认「交易行管理」入口可用、配置修改与筛选查询流程正常。
...
- 管理员中心新增「交易行管理」，支持以下能力：
  - 交易流水按成交时间倒序展示，可基于会员昵称、真实姓名、手机号或装备名称进行模糊搜索。
  - 每条记录包含买卖双方当前资料、成交价、手续费与卖家实得金额，便于运营复核。
  - 可直接在后台调整手续费比例、最短及最长挂单时间，配置保存于 `tradeSettings` 集合并实时影响后续操作。
- 配置修改完成后，交易云函数会自动读取最新参数；如需立即让前端表单展示新范围，可提示用户重新进入交易页以刷新缓存。
