...
Page({
  ...
  async handleCreateGuild() {
    ...
    wx.navigateTo({ url: buildTicketedUrl('/packages/guild/create/index', ticket) });
  },
  async handleViewGuild(event) {
    ...
    const url = buildTicketedUrl(`/packages/guild/detail/index?id=${encodeURIComponent(id)}`, ticket);
    wx.navigateTo({ url });
  },
  async handleTeamBattle() {
    ...
    const baseUrl = `/packages/guild/team/index?guildId=${encodeURIComponent(guild.id)}`;
