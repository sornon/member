...
- 感恩节购票前新增实时库存/已购检查：售罄或已购时直接提示并阻断支付；支付回调若因售罄失败则友好提示且不重复发放权益，避免扣费后状态不一致。
- 感恩节砍价新增档案完成奖励：昵称非空且头像非默认占位图时自动补发一次砍价机会并落盘 `thanksgivingProfileRewarded`，防止重复发放；同步更新活动与部署文档说明默认头像路径校验与回填策略。
- 修复 `bhkBargainStock` 读取异常会被误判为“未初始化”并回填默认库存的问题：区分“文档不存在”与权限/网络错误，防止审核或并发异常把库存重置为 15 席。
